...
interface Social {
  ...
  instagram?: string;
  facebook?: string;
  [key: string]: string | undefined; // allow other optional social keys if needed
}
...
export default function Team() {
  // give useState the TeamMember type so selected.social is known shape
  const [selected, setSelected] = useState<TeamMember>(team[0]);
  const phoneNumber = selected.phoneNumbers[0];
  const whatsappNumber =
    selected.phoneNumbers.length > 1 ? selected.phoneNumbers[1] : phoneNumber;

  // now these are properly typed
  ...
  const hasEmail = !!(selected.email && selected.email.length > 0);
        ...
        <div className="bg-white rounded-[28px] md:rounded-[36px] flex flex-row min-h-[480px] sm:min-h-[560px] md:h-[720px] lg:h-[900px] shadow-xl overflow-hidden">
          ...
          <div className="w-[60px] sm:w-[72px] md:w-[400px] border-r relative flex-shrink-0">
                  ...
                  <li key={i}>
                    <button
                      ...
                      className={`block w-[44px] h-[44px] rounded-full overflow-hidden ${
                        selected.name === member.name ? "p-[2px] bg-gradient-to-b from-[#B94255] to-[#0A0267]" : ""
                      }`}
                    >
                      <img
                        src={member.image}
            ...
            <button
              disabled={!canScroll}
              onClick={() =>
                document.querySelector(".custom-scroll")?.scrollBy({ top: 120, behavior: "smooth" })
              }
              className={`absolute bottom-3 left-1/2 -translate-x-1/2 z-50 text-black text-xl font-semibold transition ${
                canScroll ? "hover:scale-125 cursor-pointer" : "opacity-30 cursor-not-allowed"
              }`}
            >
              ▼
            </button>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div>
                ...
                <div className="mt-3 flex items-center gap-3 md:hidden">
                  {whatsappNumber && (
                    <a href={`https://wa.me/${whatsappNumber}`} target="_blank" className="w-8 h-8 bg-white flex items-center justify-center" rel="noreferrer">
                  ...
                  {hasLinkedIn && (
                    <a href={selected.social.linkedin} target="_blank" className="w-8 h-8 bg-white flex items-center justify-center" rel="noreferrer">
            ...
            <div className="mt-6 hidden md:block text-black">
              ...
              <div className="mt-4 flex flex-wrap gap-x-10 gap-y-5 text-sm md:text-base">
                ...
                {whatsappNumber && (
                  <div className="flex items-center gap-2">
                    <img src="/assets/wats.png" className="w-5 h-5" />
                    <a href={`https://wa.me/${whatsappNumber}`} target="_blank" className="hover:underline" rel="noreferrer">
                      {whatsappNumber}
                    </a>
                  </div>
                )}
                ...
                {hasLinkedIn && (
                  <div className="flex items-center gap-2">
                    <img src="/assets/linkedin.png" className="w-5 h-5" />
                    <a href={selected.social.linkedin} target="_blank" className="hover:underline" rel="noreferrer">
                      LinkedIn
                    </a>
                  </div>
                )}
                ...
                {hasInstagram && (
                  <div className="flex items-center gap-2">
                    ...
                    <a target="_blank" href={selected.social.instagram} className="hover:underline" rel="noreferrer">
                      Instagram
                    </a>
                  </div>
                )}
                ...
                {hasFacebook && (
                  <div className="flex items-center gap-2">
                    ...
                    <a target="_blank" href={selected.social.facebook} className="hover:underline" rel="noreferrer">
                      Facebook
                    </a>
                  </div>
                )}
